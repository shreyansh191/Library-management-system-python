class Library:
    ...
    def after_resv(self, name, bookname):
    ...
    def borrowBook(self, name, bookname):
        ...
        elif self.after_resv(name, bookname):
    ...
    def bartobook(self, bar):
        i_b=int(bar.find('b'))
        bookname=self.lstbook[int(bar[:i_b])][0]
        bookitem=bar[int(bar[i_b+1:])]
        return bookname
    ...
    def returnBook(self):
        ...
        for key in date_tr:
            if name in key:
                tgl_s=False
                count+=1
                print(f"{count}. Book '{Library.bartobook(lib,key[1])}' with barcode '{key[1]}' checked out on date {date_tr[key]}")
        ...
        self.book = Library.bartobook(lib,bar_in)
...
if __name__ == "__main__":  
    ...
    while (True):
        ...
        try:
            ...
            elif action == -1:  #lstbook every thing regarding book/s 
                print('lib.lstbook:\n',lib.lstbook)
